...
const path_2 = __nccwpck_require__(1017);
...
class GitHubDownloader {
    ...
    downloadFolder() {
        return new Promise((resolve, reject) => {
            this.getFileStructure(this.options.input_path, this.options.repository).then(files => {
                Promise.all(files
                    .filter(node => node.path.startsWith(this.options.input_path) &&
                    node.type === 'file')
                    .filter(node => {
                    return this.options.filter
                        ? (this.options.filter.extensions || [(0, path_1.extname)(node.path)]).includes((0, path_1.extname)(node.path)) &&
                            (this.options.filter.files || [node.path]).includes(node.path)
                        : true;
                })
                    .map(async (node) => this.getFileContent(this.options.repository, node))).then(files => {
                    Promise.all(files.map(file => {
                        return mkdir((0, path_2.dirname)((0, path_2.join)(this.options.output_path || '', file.path)), {
                            recursive: true
                        })
                            .then(() => {
                            return writeFile((0, path_2.join)(this.options.output_path || '', file.path), file.contents);
                        })
    ...
    async getFileContent(repository, node) {
        ...
        return {
            path: node.path,
            contents: Buffer.from(data.content)
        };
    }
...
async function run() {
    try {
        ...
        const INPUT_PATH = core.getInput('input_path');
